...
const rgbPattern = /^\s*rgb\s*\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*\)\s*$/;
const rgbaPattern = /^\s*rgba\s*\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]*\.?[0-9]*)\s*\)\s*$/;
...
function parse(colorText) {
    if (colorText) {
        if (colorText.startsWith("#")) {
            ...
            const m = colorText.match(rgbPattern);
            if (m) {
                return {
                    red: parseInt(m[1]), // r
                ...
                const m1 = colorText.match(rgbaPattern);
                if (m1) {
                    let alpha;
                    if (m1[4] === '.')
                        alpha = 0;
...
class Color {
    constructor(aRed, aGreen, aBlue, aAlpha) {
        let _red = 0, _green = 0, _blue = 0, _alpha = 0xff;
        if (arguments.length === 1) {
            const _color = parse(`${aRed}`);
            if (_color) {
                _red = _color.red;
                _green = _color.green;
                _blue = _color.blue;
